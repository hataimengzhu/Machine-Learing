...
def sklearnData(model,noise=None):#sklearn数据集
    ...
    return dataSet, labels, dataDic
class SVM:
    def __init__(self,features,labels,dataDic,c,kernel,maxiter):
    ...
    def kkt(self,order):#KTT条件要求
        ...
        if 0<self.Alphas[order]<self.C:
            ...
            return yiEi<0
    def f(self,i):
        # xishu = np.multiply(self.Alphas[self.Nzero], self.Y[self.Nzero]).T  # (m*1).T lambdai*yi
    ...
    def train(self,show=0,model=None):
        ...
        while pairChanged!=0 and iter<self.MaxIter:#迭代更新alpha
            ...
            # self.Alphas[self.Alphas < 0] = 0#没办法，没有裁剪AjNew,可能会导致AjNew<0,但裁剪的化，又会导致sum(lambdai*yi)!=0
            iter +=1
        ...
        #存储支持向量
        self.Vector = np.where((self.Alphas.A>0) & (self.Alphas.A<=self.C))
        end = time.time()  # 计时结束
    ...
    def interval(self,xy):# 计算点到决策边界距离
        ...
        dataset = np.mat(xy)
        # xishu = np.multiply(self.Alphas[self.Nzero], self.Y[self.Nzero]).T
        ...
        d = tmp/wNorm2
        d = np.array(sum(tmp.getA().tolist(), [])).reshape([int(np.sqrt(xy.shape[0]))]*2)
        return d, wNorm2
    def check(self,deviation=None):#检查kkt条件：alphai>=0,sum(alphai*yi)=0,是否满足
        ...
        if deviation is None:
            accuracy = abs(predict - self.Y) == 0.0
        else:
    ...
    def display(self,oneZero):
        ...
        # 设置Y轴标签
        plt.ylabel('x2')
        plt.xlim([min(self.X[:,0]).getA()[0][0]-10, max(self.X[:,0]).getA()[0][0]+10])
        plt.ylim([min(self.X[:,1]).getA()[0][0]-10, max(self.X[:,1]).getA()[0][0]+10])
        ...
        for i, alpha in enumerate(self.Alphas):
            ...
            # elif alpha == 0:
            #     x, y = self.X[i, 0], self.X[i, 1]
                # plt.scatter(x, y, s=100, c='none', alpha=0.7,linewidth=1.5, edgecolor='y',marker='p')
                plt.text(x + 0.01, y + 0.04, str('%.0f' % alpha.getA()[0][0]), ha='center', va='bottom', fontsize=7.5)
        # 要画决策边界，必须要有网格
...
if __name__=='__main__':
    #加载数据集
    DataSet, Labels, DataDic = loadData('./Data/'+'testSet.txt')
    # DataSet, Labels, DataDic = sklearnData('moons',0.1)#moons/circles/blobs/classification/gaussian_quantile

    C= 0.2#float('inf')/0.6 惩罚因子
    Kernel = ['rbf',3]#['linear',None]/['poly',[1/DataSet.shape[0],3]]]/['rbf',3]/['sigmoid',[1/DataSet.shape[0],0]]#核函数
    interation = 300#最大迭代次数
    svm= SVM(DataSet, Labels, DataDic,C,Kernel,interation)#SVM实例化
    # Model = 'random'#random/Ej_Ei/eta!=0 #select j方式
    display = 0#是否动态显示分类过程
    svm.train(display)#训练
    # svm.sklearn('poly')#调用sklearn库分类
